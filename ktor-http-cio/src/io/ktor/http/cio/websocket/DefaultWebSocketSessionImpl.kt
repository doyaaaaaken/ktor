...
class DefaultWebSocketSessionImpl(
    ...
    private val executionContext: CoroutineContext = ioCoroutineDispatcher,
    ...
    init {
        ...
        val ponger = ponger(executionContext, this, pool)
        ...
        parent.invokeOnCompletion {
            launch(executionContext, start = CoroutineStart.UNDISPATCHED) {
    ...
    private suspend fun sendCloseSequence(reason: CloseReason?) {
        if (closeReasonRef.isCompleted) return
    ...
    private fun runPinger() {
        if (!closeReasonRef.isCompleted) {
            val newPinger = pingInterval?.let { interval ->
                pinger(executionContext, raw, interval, timeout, raw.outgoing, pool)
            }
